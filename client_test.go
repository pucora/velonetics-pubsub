...
func TestNew_subscriber(t *testing.T) {
	...
	if log := buff.String(); strings.HasSuffix(log, "DEBUG: [BACKEND: mem://host/subscriber-topic-url][PubSub] Subscriber initialized successfully") {
...
func TestNew_publisher_unknownProvider(t *testing.T) {
	...
	lines := strings.Split(buff.String(), "\n")
	if !strings.HasSuffix(lines[0], "ERROR: [BACKEND: schema://host/publisher-topic-url][PubSub] Error while opening topic: open pubsub.Topic: no driver registered for \"schema\" for URL \"schema://host/publisher-topic-url\"; available schemes: awssns, awssqs, azuresb, gcppubsub, kafka, mem, nats, rabbit") {
		t.Error("unexpected first log line:", lines[0])
	}
	if lines[1] != "" {
		t.Error("unexpected final log line:", lines[1])
